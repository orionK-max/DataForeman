...
# Create docker-compose.override.yml for Windows compatibility
# On Windows, Docker's host network mode doesn't work like Linux
# We need to use bridge networking and adjust service connections
...
# This file configures networking for Windows/Docker Desktop compatibility

services:
  connectivity:
    # Override host network mode (not supported on Windows)
    network_mode: bridge
    ports:
      - "3100:3100"
    environment:
...
"@
    ...
    try {
        ...
        Write-ColorOutput "✓ Created docker-compose.override.yml for Windows compatibility" "Green"
    } catch {
        Write-ColorOutput "⚠ Could not create docker-compose.override.yml" "Yellow"
    }
} else {
    Write-ColorOutput "✓ docker-compose.override.yml already exists" "Green"
}
Write-Host ""
